...
export default function Lunch() {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        ...
        if (matchedUser?.lunchTime) {
          // Convert Firestore Timestamp to JS Date
          const dateObj = matchedUser.lunchTime.toDate();
          const formattedTime = dateObj.toLocaleString("en-US", {
            dateStyle: "medium",
            timeStyle: "short",
          });
          toast.error(
            `${matchedUser.fullName} has already checked in for lunch.`,
            {
              position: "top-center",
              autoClose: 4000,
              hideProgressBar: false,
      ...
      try {
        if (userData?.id && userData?.entryTime) {
          ...
          await updateUserDataById(userData.id, {
            lunchTime: serverTimestamp(),
          });
          console.log("Exit time updated");

          toast.success(
            `${userData.fullName} successfully checked in for lunch.`,
            {
              position: "top-center",
              autoClose: 2000,
              hideProgressBar: false,
  ...
  return (
    <div className="Entry w-100 h-100  flex-col-center justify-content-start px-4 py-5">
      ...
      <h1 className="text-center w-100">Scan at Lunch Point</h1>
